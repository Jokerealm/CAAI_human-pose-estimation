...
def combined_depth_consistency_loss(x: torch.Tensor, gt: torch.Tensor, lambda_spatial=1.0, lambda_temporal=1.0):
    '''
    组合空间和时间关系深度一致性损失函数
    ...
    x: 预测的3D姿态 [B, N, J, 3]
    gt: 真实的3D姿态 [B, N, J, 3]
    lambda_spatial: 空间损失的权重系数
    lambda_temporal: 时间损失的权重系数
    
    返回:
    total_loss: 总损失值
    spatial_loss_val: 空间损失值
    temporal_loss_val: 时间损失值
    '''
    # 计算空间关系深度一致性损失
    spatial_loss_val = loss_spatial_rank(x, gt)
    
    # 计算时间关系深度一致性损失
    temporal_loss_val = loss_temporal_rank(x, gt)
    
    # 组合损失
    total_loss = lambda_spatial * spatial_loss_val + lambda_temporal * temporal_loss_val
    
    return total_loss, spatial_loss_val, temporal_loss_val