...
from common import eval_cal
from common.utils import save_model_epoch
from common.utils_3dhp import *
from common.opt_1 import opts
...
def step(split, opt, actions, dataLoader, model, optimizer=None, epoch=None, writer=None, adaptive_weight=None):
    ...
    action_error_sum = define_error_list_3dhp(actions)
    # print(actions)
    if split == 'train':
        model.train()
    else:
    ...
    for i, data in TQDM:
        ...
        # print(action)
        # [b, 27, 4, 17, 2], [b, 27, 17, 3], [b, 9], [b], [b, 4], [b, 4, 17, 17]
        ...
        if split == 'train':
            ...
            output_3D[:, :, 0, :] = 0
            # print(f"output_3D:{output_3D}, out_target:{out_target}")
            action_error_sum = eval_cal.test_calculation(output_3D, out_target, action, action_error_sum, opt.dataset, subject)
            # print("error_sum:",action_error_sum)
...
if __name__ == '__main__':
    ...
    if opt.train:
        train_data = Fusion_3dhp(opt=opt, train=True)
        print('train_data:', len(train_data))
        train_dataloader = torch.utils.data.DataLoader(train_data, batch_size=opt.batch_size,
                                                       shuffle=True, num_workers=int(opt.workers), pin_memory=True)

    test_data = Fusion_3dhp(opt=opt, train=False)
    
    test_dataloader = torch.utils.data.DataLoader(test_data, batch_size=opt.batch_size,
                                                  shuffle=False, num_workers=int(opt.workers), pin_memory=True)

    model = sgraformer(num_frame=opt.frames, num_joints=17, in_chans=opt.in_channels, embed_dim_ratio=32, depth=4,
