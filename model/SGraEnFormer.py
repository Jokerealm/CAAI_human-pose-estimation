...
from HyperGraph.EModule import Enhance_Module
...
class sgraformer(nn.Module):
    def __init__(self, num_frame=9, num_joints=17, in_chans=2, embed_dim_ratio=32, depth=4,
        ...
        Args:
            ...
            feature_dim (int): feature dimension for multi-granularity enhancement
            dataset (str): dataset name (h36m, mpi, etc.)
        """
        super().__init__()
        ...
        # 多粒度特征增强模块
        self.multi_granularity_enhancer = Enhance_Module(
    ...
    def forward(self, x, hops): # hop:[b, 4, 17, 17]
        ...
        # 应用多粒度特征增强模块
        # 重塑输入为 [BatchSize, Number_of_Views, Number_of_Frames, Number_of_Joints, 2]
        x_reshaped = rearrange(x, 'b f v j c -> b v f j c')
        # 应用增强模块
        x_enhanced = self.multi_granularity_enhancer(x_reshaped)
        # 重塑回原始形状 [BatchSize, Number_of_Frames, Number_of_Views, Number_of_Joints, 2]
        x = rearrange(x_enhanced, 'b v f j c -> b f v j c')

        # Store original requires_grad status to restore later
