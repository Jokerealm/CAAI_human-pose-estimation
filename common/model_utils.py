...
if __name__ == '__main__':
    ...
    # 加载预训练权重（在DataParallel之前）
    if opt.previous_dir != '':
        print('Loading pretrained model:', opt.previous_dir)
        
        from common.model_utils import load_pretrained_weights
        loaded_keys, missing_keys, unexpected_keys = load_pretrained_weights(
            model, opt.previous_dir, strict=False, verbose=False)
        
        print(f'Loaded {loaded_keys} keys from checkpoint')
        
        # 检查是否有temporal_refine相关的missing keys
        refinement_missing = [k for k in missing_keys if 'temporal_refine' in k]
        if refinement_missing and hasattr(opt, 'train_refinement_only') and opt.train_refinement_only:
            print(f'temporal_refine module will be randomly initialized ({len(refinement_missing)} new parameters)')
    
    # 如果启用refinement-only训练，冻结其他层
    if opt.train and hasattr(opt, 'train_refinement_only') and opt.train_refinement_only:
        print('\nRefinement-only training mode enabled')
        
        from common.model_utils import freeze_all_except_refinement
        
        # 检查模型是否有temporal_refine模块
        actual_model = model.module if hasattr(model, 'module') else model
        if not hasattr(actual_model, 'temporal_refine'):
            print('Error: Model does not have temporal_refine module')
            exit(1)
        
        # 冻结除temporal_refine外的所有层
        freeze_all_except_refinement(model, verbose=False)
        
        trainable_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
        total_params = sum(p.numel() for p in model.parameters())
        print(f'Trainable parameters: {trainable_params/1e6:.2f}M / {total_params/1e6:.2f}M ({trainable_params/total_params*100:.1f}%)')
    ...
    # trainable_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    # print(f"\n Model Parameters:")
    # print(f"  Total:     {total_params / 1e6:>8.2f}M")
    # print(f"  Trainable: {trainable_params / 1e6:>8.2f}M ({trainable_params/total_params*100:.1f}%)")
    # print(f"  Frozen:    {(total_params - trainable_params) / 1e6:>8.2f}M ({(total_params - trainable_params)/total_params*100:.1f}%)\n")

    # 设置优化器
    all_param = []
    lr = opt.lr
    
    # 如果是refinement-only模式，调整学习率并只优化可训练参数
    if opt.train and hasattr(opt, 'train_refinement_only') and opt.train_refinement_only:
        lr = opt.lr * opt.refinement_lr_ratio
        all_param = [p for p in model.parameters() if p.requires_grad]
        
        if len(all_param) == 0:
            print('Error: No trainable parameters found')
            exit(1)
        
        print(f"Learning rate: {lr:.6f} (base_lr × {opt.refinement_lr_ratio})")
    else:
