...
def freeze_modules(model, freeze_agf=True):
    ...
    # 冻结AGF相关模块
    if freeze_agf:
        if hasattr(actual_model, 'temporal_att'):
            for param in actual_model.temporal_att.parameters():
                param.requires_grad = False
        if hasattr(actual_model, 'temporal_tcn'):
            for param in actual_model.temporal_tcn.parameters():
                param.requires_grad = False
    else:
        # 解冻模块
        if hasattr(actual_model, 'temporal_att'):
